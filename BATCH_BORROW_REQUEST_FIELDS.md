...
|-------|------|----------|-------------|
| `batchId` | `String` | ✅ Yes | Unique identifier shared by all items in the same batch. Used to group requests together. |
| `batchSize` | `int` | ✅ Yes | Total number of items in this batch. Same value for all requests in the batch. |

### **Request Identification Fields**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `requestId` | `String` | ✅ Yes | Unique identifier for this specific request. Auto-generated by Firebase. |
| `userId` | `String` | ✅ Yes | User ID of the student making the request. |
| `userEmail` | `String` | ✅ Yes | Email address of the student making the request. |

### **Equipment Information Fields**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `itemId` | `String` | ✅ Yes | Unique identifier of the equipment item. |
| `categoryId` | `String` | ✅ Yes | Unique identifier of the equipment category. |
| `itemName` | `String` | ✅ Yes | Name of the equipment item. |
| `categoryName` | `String` | ✅ Yes | Name of the equipment category. |
| `itemNo` | `String` | ✅ Yes | Item number. Format: `LAB-{first5CharsOfItemId}` (e.g., "LAB-ABCDE"). |
| `quantity` | `int` | ✅ Yes | Quantity of items requested for this specific equipment. |

### **Laboratory Fields**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `laboratory` | `String` | ✅ Yes | Display name of the laboratory (e.g., "Chemistry Laboratory"). Used for backward compatibility. |
| `labId` | `String` | ✅ Yes | Laboratory code (e.g., "LAB001"). |
| `labRecordId` | `String` | ✅ Yes | Firebase record ID of the laboratory. |

### **Schedule Fields**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `dateToBeUsed` | `String` | ✅ Yes | ISO 8601 date string when the equipment will be used. Format: `"2024-01-15T00:00:00.000Z"`. |
| `dateToReturn` | `String` | ✅ Yes | ISO 8601 date string when the equipment should be returned. Format: `"2024-01-20T00:00:00.000Z"`. |

### **Adviser Fields**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `adviserName` | `String` | ✅ Yes | Name of the adviser/teacher. |
| `adviserId` | `String` | ✅ Yes | User ID of the adviser/teacher. |

### **Status Fields**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `status` | `String` | ✅ Yes | Request status. Initial value: `"pending"`. Possible values: `pending`, `approved`, `rejected`, `released`, `returned`. |
| `requestedAt` | `String` | ✅ Yes | ISO 8601 timestamp when the request was created. Format: `"2024-01-10T10:30:00.000Z"`. |
...
| `releasedBy` | `String` | ❌ No | User ID of the person who released the item. Added by web admin. |
| `returnedAt` | `String` | ❌ No | ISO 8601 timestamp when the item was returned. Added by student or web admin. |
...
## 🔍 Key Differences: Batch vs Single Request
...
- ❌ Does NOT have `batchSize` field
- ✅ Has `signature` field (optional)
- One request created
